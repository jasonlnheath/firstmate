...
Pi gates the fresh-worktree case too; a crewmate or scout spawn grants trust for that run with `--approve`, while a secondmate launch still meets the dialog and, unlike Claude's, it is answered with Enter - `references/harness/pi.md` owns that recipe and where the decision persists.
