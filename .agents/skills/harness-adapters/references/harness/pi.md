...
The seed carries a symlinked `auth.json` to the operator's own store (never a copy, re-established every launch, launch refused when the store is missing), a symlinked `models.json` when the operator has one (Pi reads the custom provider catalog from the agent dir only, so without it a crew-dispatch rule naming a local or proxied provider would resolve to no provider and silently fall back to a cloud model; Pi's derived `models-store.json` lands beside it), a symlinked `extensions/herdr-agent-state.ts` when the herdr-managed Pi integration is installed (it reports `agent_status` to `herdr agent get` and disables itself outside a herdr pane), and a telemetry-off `settings.json`; the two optional links are dropped again on the next launch once their source is gone, and worker session transcripts land under the seed's `sessions/` by design, retained rather than auto-deleted.
