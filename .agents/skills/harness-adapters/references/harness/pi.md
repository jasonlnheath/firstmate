...
| Model flag | `--model <provider>/<id>` on a crewmate or scout launch: omitting it is refused rather than left to Pi's own default, and a bare id or `id:level` pattern (which Pi 0.85.1 itself would resolve against its catalog) is refused too, because the credential check is scoped to the provider the pin names (see Task-worker launch hardening below); a secondmate may omit it or pass any `--model <pattern>` Pi accepts. |
...
The seed carries a symlinked `auth.json` to the operator's own store when one exists (never a copy, re-established every launch; the launch is refused - by `bin/fm-harness.sh validate-worker-credentials`, before any endpoint, worktree slot, or task record exists, and on the pre-stop side of a relaunch - when no credential source Pi could use for the pinned model's provider is present: the store is missing or holds no entry for that provider - a never-authenticated operator's store is the `{}` that 0.85.1 writes on `pi --help` when it initializes a missing agent dir, which the spawn's own TUI-mode probe triggers before the seed runs - AND that provider carries no `apiKey` of its own in the operator's `models.json` (0.85.1 accepts a literal, the dummy value a keyless local server needs, `$ENV` interpolation, or a `!command`, none of which touches `auth.json`) AND that provider, when built in, has no credential set in the environment variable Pi reads for it (the `pi-ai` `env-api-keys` map, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `ZAI_API_KEY`, and so on, checked in the spawning process's environment); any one of the three lets the worker run, all three are scoped to the provider the pin names, so a pin nothing keys is refused up front rather than left to die in the pane, and `codex-native` is exempt because the pi-codex-native adapter authenticates through the Codex App Server's own login rather than Pi's store (`tests/fm-pi-codex-native.test.sh` runs it on an agent dir with no `auth.json`); one accepted cost: 0.85.1 serializes credential mutation with a lockfile beside the `auth.json` path it was given without resolving symlinks, so a worker locks `state/pi-worker-agent/auth.json.lock` while the operator's own Pi locks `~/.pi/agent/auth.json.lock`, and for an OAuth-backed provider a refresh the operator session and a worker both start inside the five-minute expiry window is no longer serialized between them, which can fail one side's turn with `OAuth refresh failed` and, where the provider rotates refresh tokens, leave the store holding a rotated-out token that forces a re-login; workers still share one lock among themselves, and an api-key-only store such as this host's is unaffected), a symlinked `models.json` when the operator has one (Pi reads the custom provider catalog from the agent dir only, so without it a crew-dispatch rule naming a local or proxied provider would fail `--model` resolution, which 0.85.1 reports as a startup error and exits 1 on before the TUI, leaving a dead pane for the start gate to time out on; Pi's derived `models-store.json` lands beside it and the worker refreshes it itself), a symlinked `extensions/herdr-agent-state.ts` when the herdr-managed Pi integration is installed (it reports `agent_status` to `herdr agent get` and disables itself outside a herdr pane), and no `settings.json` of its own (Pi consults that file for telemetry only when `PI_TELEMETRY` is unset, and every worker launch sets it to `0`, so Pi writes its own on first run); every link is dropped again on the next launch once its source is gone - an absent auth store included, since a launch the guard accepted through a `models.json` `apiKey` or a provider variable is one no auth store was needed for - and worker session transcripts land under the seed's `sessions/` by design, retained rather than auto-deleted.
...
Isolation does strip the operator's saved default model and thinking level, because those live in the `settings.json` the seed does not carry: a model-less launch on 0.85.1 would fall through `findInitialModel` to the first provider in Pi's own hardcoded order that has a key in `auth.json`, so `bin/fm-harness.sh validate-worker-model` refuses a crewmate or scout launch whose model is empty, `default`, or not of the `<provider>/<id>` shape before any endpoint exists, naming the fix (pin a model in `config/crew-dispatch.json` or pass `--model <provider>/<id>`); the shape requirement exists so the credential guard above can be scoped to the pinned provider, and `bin/fm-control.sh relaunch` runs both validators before the running worker is stopped; secondmate launches are exempt because their panes are primaries on the operator's own agent dir.
