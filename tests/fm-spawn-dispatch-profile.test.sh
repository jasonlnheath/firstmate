...
test_pi_threads_model_and_max_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi PI_TELEMETRY=0 PI_OFFLINE=1 PI_SKIP_VERSION_CHECK=1 PI_CODING_AGENT_DIR='$HOME_DIR/state/pi-worker-agent' '$FAKEBIN_DIR/pi' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' --approve --append-system-prompt" \
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed PI_TELEMETRY=0 PI_OFFLINE=1 PI_SKIP_VERSION_CHECK=1 PI_CODING_AGENT_DIR='$HOME_DIR/state/pi-worker-agent' '$FAKEBIN_DIR/pi-signed' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' --approve --append-system-prompt" \
...
test_pi_signed_persistent_secondmate_uses_pi_extensions_and_identity() {
  ...
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed PI_TELEMETRY=0 PI_OFFLINE=1 PI_SKIP_VERSION_CHECK=1 '$FAKEBIN_DIR/pi-signed' --tui-mode regular -e '$sm/.pi/extensions/fm-primary-turnend-guard.ts' -e '$sm/.pi/extensions/fm-primary-pi-watch.ts'" \
