...
fm_tmux_composer_state() {  # <target> -> empty|pending|pending-unproven|unknown
  local target=$1 cy pane verdict identity _fm_caps
  ...
  _fm_caps=$(fm_tmux_composer_caps)
  verdict=$(fm_composer_classify_screen "$_fm_caps" "$pane" "$cy")
  if [ "$verdict" = need-identity ]; then
    if ! identity=$(fm_tmux_composer_identity "$target") || [ -z "$identity" ]; then
      identity=probe-absent
    fi
    verdict=$(fm_composer_classify_screen "$_fm_caps" "$pane" "$cy" "$identity")
  ...
  if [ "$verdict" = unknown ] && fm_tmux_pane_is_cursor "$target"; then
    verdict=$(fm_composer_classify_screen "$_fm_caps" "$pane" '')
  fi
  printf '%s' "$verdict"
}
