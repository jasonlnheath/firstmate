...
Pi hands each wake's delivery to a background task once its own successor is verified, so the next close's successor is never serialized behind that delivery, and a background delivery failure surfaces through the same typed failure channel.
Pi's branch settlement is bounded (`FM_WATCH_BRANCH_SETTLEMENT_TIMEOUT_MS`, default 45000): past the bound, or on a rejected settlement, the wake is delivered to main, and the durable wake queue plus branch-grant reconciliation deduplicate the wake the branch may still be settling.
...
The same suite proves the next close's successor is restored while the previous wake's delivery is still in flight, that a verified successor failing mid-delivery is retried immediately without waiting for that delivery, and that a branch settlement outliving its bound falls back to main without duplicating the late settlement.
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows including predecessor-pid linkage, pre-trap interruption rows, restart TERM-outcome rows, and double-start winner marking, and a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination.
