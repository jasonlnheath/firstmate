...
- An adapter that is not verified for this task's kind is refused **before** the running agent is stopped, not after.
  ...
  The same pre-stop mirroring covers every launch-side profile refusal `bin/fm-harness.sh` owns: a `pi` or `pi-signed` ship or scout whose resolved model is empty, `default`, or not `<provider>/<id>` (`validate-worker-model`), one whose pinned provider has no credential the operator's Pi could use (`validate-worker-credentials`), and a native `ultra` effort on an unsupported profile (`validate-native-effort`), both refuse while the agent is still up.
