...
An omitted model or effort means the selected harness uses its own default for that axis, except that a `pi` or `pi-signed` crewmate or scout profile must name a model: the worker runs in an isolated agent dir that carries no saved default, so `bin/fm-harness.sh validate-worker-model` refuses a model-less launch before any pane opens and a model-less relaunch before the running agent is stopped (see [`docs/examples/crew-harness-pi.md`](examples/crew-harness-pi.md)).
