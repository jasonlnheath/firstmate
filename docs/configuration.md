...
The same file sets `test_agent_timeout: 120m`, because this repo's test-step validation - real arming-path drives plus a roughly 20-minute targeted-suite batch - outlasts no-mistakes' default 30-minute agent budget; the larger budget skips nothing and only lets the same validation finish.
...
See [`docs/examples/crew-harness-pi.md`](examples/crew-harness-pi.md) for a thin Pi-first example of this file.
...
An omitted model or effort means the selected harness uses its own default for that axis, except that a `pi` or `pi-signed` crewmate or scout profile must name a model as `<provider>/<id>`: the worker runs in an isolated agent dir that carries no saved default, and the launch is checked against that provider's credentials, so `bin/fm-harness.sh validate-worker-model` and `validate-worker-credentials` refuse a model-less, provider-less, or credential-less launch before any pane opens and the same relaunch before the running agent is stopped (see [`docs/examples/crew-harness-pi.md`](examples/crew-harness-pi.md)).
...
FM_WATCH_BRANCH_SETTLEMENT_TIMEOUT_MS=45000   # milliseconds the Pi watcher extension bounds a supervision-branch settlement before delivering the wake to main
