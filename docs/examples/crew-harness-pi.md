...
The pinned model also needs a credential Pi can reach: a `/login` entry in the operator's `auth.json`, a provider in the operator's `models.json` carrying its own `apiKey` (a dummy value is enough for a keyless local server), or the provider's environment variable such as `OPENAI_API_KEY`; a launch with none of the three is refused before any pane opens.
