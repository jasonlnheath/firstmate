...
| `fm-helm-reset.sh`       | Gated bedtime reset: /new the primary pi helm from the lock-holder pane and submit the stowed-handoff continuation prompt when every gate passes |
...
| `fm-harness.sh`          | Detect the running harness, resolve crew or secondmate harness, model, and effort, validate the native-only `ultra` effort, and refuse a pi or pi-signed crewmate or scout launch whose model is not a `<provider>/<id>` pin or whose pinned provider has no credential |
