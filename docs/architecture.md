...
After those three probes all come back negative and immediately before the unchanged escalation, a home holding `TYPESAFE_API_KEY` may consult the Jev pre-screen (`bin/fm-jev-screen.sh`) with the pane tail the poll already captured for hashing, at the same once-per-`STALE_ESCALATE_SECS` budget the probes observe.
A high-confidence `actively-working` verdict defers exactly one threshold window through the same deferral shape a written worktree uses, but with a hard count instead of evidence that renews: a second consecutive defer without an intervening wake escalates anyway, and the budget restarts only on a pane hash change, a delivered re-surface wake, or one of the resets that already clears the write-deferral chain.
Every other outcome - no key, a below-floor or non-working verdict, a timeout, an HTTP error, or a malformed response - falls through to the identical escalation, so the classifier only ever defers and never suppresses a wake; [`configuration.md`](configuration.md) "Jev wedge pre-screen" owns the floor knob and `bin/fm-watch.sh`'s `wedge_defer_jev` owns the marker and bound mechanics.
...
Allocation carries the matching refusal: a spawn handed a pool slot a live task record of this home still holds refuses to launch and names the holder, because a lapsed process lease is not proof a slot is free; `fm_treehouse_slot_live_holder` in [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns the screen, pinned by [`tests/fm-spawn-slot-live-holder.test.sh`](../tests/fm-spawn-slot-live-holder.test.sh).
