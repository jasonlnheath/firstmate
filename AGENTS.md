...
config/jev-wedge-floor  optional confidence floor (one decimal 0-1, default 0.8) for the watcher's Jev wedge pre-screen deferral; LOCAL, gitignored; see docs/configuration.md "Jev wedge pre-screen"
config/cmux-socket-password  optional cmux control-socket password; LOCAL, gitignored; read fresh on every cmux CLI call and passed through without ever overriding an operator's own ambient CMUX_SOCKET_PASSWORD when absent (docs/cmux-backend.md "Setup")
config/wedge-alarm  optional away-mode wedge-alarm active-alert directives; LOCAL, gitignored; absent means auto (macOS Notification Center when available); see docs/wedge-alarm.md
config/helm-reset  gated bedtime /new reset for the primary pi helm (away posture, empty wake queue, no held leases, no live non-done workers, and the time window must all pass; dry-run by default, herdr only); LOCAL, gitignored, not inherited, absent = feature off; timer opt-in via bin/fm-helm-reset.sh --print-unit; see docs/configuration.md "Helm reset"
...
state/               runtime records and signals; gitignored
  ...
  .helm-reset.refusal .helm-reset.last .helm-reset.lock  the latest gated helm-reset refusal (gate + reason, for morning diagnosis), the last completed dry-run or real reset record, and the transient run lock serializing concurrent resets; bin/fm-helm-reset.sh
